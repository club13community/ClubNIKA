...
#define MONITORING_PERIOD_ms	3000U
#define MAX_TASK_NUM			30U
#define STACK_SIZE				70U
...
static void monitor_stack(void * args) {
	while (true) {
		...
		if (num == 0) {
			// todo log too many tasks
			continue;
		}
		uint8_t max_usage = 0; // max. used stack, %
		UBaseType_t max_usage_ind = 0;
		for (UBaseType_t i = 0; i < num; i++) {
			...
			// todo log stack usage
			if (max_usage < stack_usage) {
				max_usage = stack_usage;
				max_usage_ind = i;
			}
		}
		// todo log max stack usage
		TaskStatus_t s = task_statuses[max_usage_ind];
		vTaskDelay(pdMS_TO_TICKS(MONITORING_PERIOD_ms));
	}
}
